...
def search(request):
    ...
    summaries = Resource.objects.filter(title__icontains=query).filter(resourcetype = "Summary") | Resource.objects.filter(major__icontains=query).filter(resourcetype = "Summary")
    exams = Resource.objects.filter(title__icontains=query).filter(resourcetype = "Exam") | Resource.objects.filter(major__icontains=query).filter(resourcetype = "Exam")
